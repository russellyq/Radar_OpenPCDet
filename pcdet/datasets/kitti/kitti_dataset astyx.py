...
class KittiDataset(DatasetTemplate):
    ...
    #         for x in new_line:
    #             arr.append(float(x))
    #     arr = np.array(arr).reshape((-1, 6))[:, 0:4]
        
    #     # Encode intensity (for lidar) and radial velosity (for radar)
    #     minv, maxv = min(arr[:, 3]), max(arr[:, 3])
    #     arr[:, 3] = (arr[:, 3] - minv) / (maxv - minv)
    #     return arr
    ...
    def get_lidar(self, idx):
        ...
        for line in lines:
            ...
            for x in new_line:
                arr.append(float(x))
        arr = np.array(arr).reshape((-1, 5))[:, 0:4] # X Y Z V_r
        
        # Encode  radial velosity (for radar)
        minv, maxv = min(arr[:, 3]), max(arr[:, 3])
        arr[:, 3] = (arr[:, 3] - minv) / (maxv - minv)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        ROOT_DIR = Path('/home/newdisk/yanqiao/dataset/radar_dataset_astyx/dataset_astyx_hires2019/')
        create_kitti_infos(
