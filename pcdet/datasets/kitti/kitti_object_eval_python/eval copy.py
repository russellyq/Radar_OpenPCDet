...
def _prepare_data(gt_annos, dt_annos, current_class, difficulty, custom_method=0):
    gt_datas_list = []
    dt_datas_list = []
    total_dc_num = []
    ...
    for i in range(len(gt_annos)):
        ...
        if custom_method == 3:  # results in the ROI
            rets = clean_data(gt_annos[i], dt_annos[i], current_class, difficulty, roi_clean=True)
...
def eval_class(gt_annos,
    ...
    for m, current_class in enumerate(current_classes):
        for l, difficulty in enumerate(difficultys):
            rets = _prepare_data(gt_annos, dt_annos, current_class, difficulty, custom_method=custom_method)
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    mAPbbox, mAPbev, mAP3d, mAPaos, mAPbbox_R40, mAPbev_R40, mAP3d_R40, mAPaos_R40 = do_eval(
        gt_annos, dt_annos, current_classes, min_overlaps, compute_aos, PR_detail_dict=PR_detail_dict, custom_method=0)
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(1, 2):  # min_overlaps.shape[0]):
            if compute_aos:
                if i == 1:
                    ret_dict_VoD_Entire['%s_aos_all' % class_to_name[curcls]] = mAPaos[j, 0, 1]

            if i == 1:
                ret_dict_VoD_Entire['%s_3d_all' % class_to_name[curcls]] = mAP3d[
                    j, 0, 1]  # get j class, difficulty, second min_overlap
                ret_dict_VoD_Entire['%s_bev_all' % class_to_name[curcls]] = mAPbev[
                    j, 0, 1]  # get j class, difficulty, second min_overlap
    result+=print_str("Results: \n"
            f"Entire annotated area: \n"
            f"Car: {ret_dict_VoD_Entire['Car_3d_all']} \n"
            f"Pedestrian: {ret_dict_VoD_Entire['Pedestrian_3d_all']} \n"
            f"Cyclist: {ret_dict_VoD_Entire['Cyclist_3d_all']} \n"
            f"mAP: {(ret_dict_VoD_Entire['Car_3d_all'] + ret_dict_VoD_Entire['Pedestrian_3d_all'] + ret_dict_VoD_Entire['Cyclist_3d_all']) / 3} \n"
            )
    
    mAPbbox, mAPbev, mAP3d, mAPaos, mAPbbox_R40, mAPbev_R40, mAP3d_R40, mAPaos_R40 = do_eval(
        gt_annos, dt_annos, current_classes, min_overlaps, compute_aos, PR_detail_dict=PR_detail_dict, custom_method=3)
    
    ret_dict_VoD_Driving = {}
    for j, curcls in enumerate(current_classes):
        # mAP threshold array: [num_min_overlap, metric, class]
        # mAP result: [num_class, num_diff, num_min_overlap]
        for i in range(1, 2):  # min_overlaps.shape[0]):
            if compute_aos:
                if i == 1:
                    ret_dict_VoD_Driving['%s_aos_all' % class_to_name[curcls]] = mAPaos[j, 0, 1]

            if i == 1:
                ret_dict_VoD_Driving['%s_3d_all' % class_to_name[curcls]] = mAP3d[
                    j, 0, 1]  # get j class, difficulty, second min_overlap
                ret_dict_VoD_Driving['%s_bev_all' % class_to_name[curcls]] = mAPbev[
                    j, 0, 1]  # get j class, difficulty, second min_overlap
    result+=print_str("Results: \n"
            f"Driving corridor area: \n"
            f"Car: {ret_dict_VoD_Driving['Car_3d_all']} \n"
            f"Pedestrian: {ret_dict_VoD_Driving['Pedestrian_3d_all']} \n"
            f"Cyclist: {ret_dict_VoD_Driving['Cyclist_3d_all']} \n"
            f"mAP: {(ret_dict_VoD_Driving['Car_3d_all'] + ret_dict_VoD_Driving['Pedestrian_3d_all'] + ret_dict_VoD_Driving['Cyclist_3d_all']) / 3} \n"
            )
