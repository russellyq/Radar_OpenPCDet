...
def clean_data(gt_anno, dt_anno, current_class, difficulty):
    ...
    for i in range(num_dt):
        ...
        distance = np.linalg.norm(dt_anno['location'][i])
        dif_dt = dist_to_dif(distance)
        # if height < MIN_HEIGHT[difficulty]:
        # if distance < MIN_HEIGHT[difficulty]:
        # if dif_dt > difficulty or dif_dt == -1:
        if dif_dt != difficulty or dif_dt == -1:
            ignored_dt.append(1)
        elif valid_class == 1:
            ignored_dt.append(0)
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    # min_overlaps = np.stack([overlap_0_7], axis=0)  # [2, 3, 3]
    # 每个类别输出4个结果
