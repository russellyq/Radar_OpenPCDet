...
def do_eval(gt_annos,
    ...
    return mAP_bev, mAP_bev_R40, mAP_3d, mAP_3d_R40
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    mAPbev, mAPbev_R40, mAP3d, mAP3d_R40 = do_eval(
        gt_annos, dt_annos, current_classes, min_overlaps, compute_aos, PR_detail_dict=PR_detail_dict)

    ret_dict = {}

    for j, curcls in enumerate(current_classes):
        result += print_str((f"{current_classes}"))
        result += print_str((f"{j}"))
        
        # mAP threshold array: [num_minoverlap, metric, class]
        # mAP result: [num_class, num_diff, num_minoverlap]
        for i in range(min_overlaps.shape[0]):
            result += print_str((f"{min_overlaps[:, :, j]}"))
            result += print_str((f"{min_overlaps[i, :, j]}"))
            result += print_str(
