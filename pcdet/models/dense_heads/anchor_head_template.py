...
# class AnchorHeadTemplate_multi(nn.Module):
#     def __init__(self, model_cfg, num_class, class_names, grid_size, point_cloud_range, predict_boxes_when_training):
#         """
#         AnchorHead模板
#         Args:
...
#         cls_preds_multi = self.forward_ret_dict['cls_preds_multi']
#         box_cls_labels = self.forward_ret_dict['box_cls_labels']
#         box_cls_labels = box_cls_labels.repeat((1, 2))
...
#         cls_preds_multi = cls_preds_multi.view(batch_size, -1, self.num_class)
#         cls_preds = torch.cat((cls_preds, cls_preds_multi), dim=1)
        
#         # print('\n cls_preds shape, ', cls_preds.shape)
#         # print('\n box_cls_labels shape, ', box_cls_labels.shape)
#         # print('\n cls_targets shape, ', cls_targets.shape)
#         # print('\n one_hot_targets shape, ', one_hot_targets.shape)
...
#         box_preds_multi = self.forward_ret_dict['box_preds_multi']
#         box_dir_cls_preds = self.forward_ret_dict.get('dir_cls_preds', None)
#         box_dir_cls_preds_multi = self.forward_ret_dict.get('dir_cls_preds_multi', None)
#         box_reg_targets = self.forward_ret_dict['box_reg_targets']
#         box_reg_targets = box_reg_targets.repeat((1, 2, 1))
#         box_cls_labels = self.forward_ret_dict['box_cls_labels']
#         box_cls_labels = box_cls_labels.repeat((1, 2))
...
#         anchors = anchors.view(1, -1, anchors.shape[-1]).repeat(batch_size, 2, 1)
#         box_preds = box_preds.view(batch_size, -1,
#                                    box_preds.shape[-1] // self.num_anchors_per_location if not self.use_multihead else
#                                    box_preds.shape[-1])
#         box_preds_multi = box_preds_multi.view(batch_size, -1,
#                                    box_preds_multi.shape[-1] // self.num_anchors_per_location if not self.use_multihead else
#                                    box_preds_multi.shape[-1])

#         box_preds = torch.cat((box_preds, box_preds_multi), dim=1)
#         # sin(a - b) = sinacosb-cosasinb
#         box_preds_sin, reg_targets_sin = self.add_sin_difference(box_preds, box_reg_targets)
#         # print('\n box_reg_targets, ', box_reg_targets.shape)
...
#             dir_logits_multi = box_dir_cls_preds_multi.view(batch_size, -1, self.model_cfg.NUM_DIR_BINS)
#             dir_logits = torch.cat((dir_logits, dir_logits_multi), dim=1)
            
#             # print('\n dir_logits shape: ', dir_logits.shape) # 8, 153600, 2
#             # print('\n dir_targets shape: ', dir_targets.shape) # 8, 153600, 2
...
#     def generate_predicted_boxes(self, batch_size, cls_preds, cls_preds_multi, box_preds, box_preds_multi, dir_cls_preds=None, dir_cls_preds_multi=None):
#         """
#         Args:
#             batch_size:
#             cls_preds: (N, H, W, C1)
...
#         batch_anchors = anchors.view(1, -1, anchors.shape[-1]).repeat(batch_size, 2, 1)
        
#         batch_cls_preds = cls_preds.view(batch_size, num_anchors, -1).float() \
#             if not isinstance(cls_preds, list) else cls_preds
#         batch_cls_preds_multi = cls_preds_multi.view(batch_size, num_anchors, -1).float() \
#             if not isinstance(cls_preds_multi, list) else cls_preds_multi 
#         batch_cls_preds = torch.cat((batch_cls_preds, batch_cls_preds_multi), dim=1)
        
#         batch_box_preds = box_preds.view(batch_size, num_anchors, -1) if not isinstance(box_preds, list) \
#             else torch.cat(box_preds, dim=1).view(batch_size, num_anchors, -1)
#         batch_box_preds_multi = box_preds_multi.view(batch_size, num_anchors, -1) if not isinstance(box_preds_multi, list) \
#             else torch.cat(box_preds_multi, dim=1).view(batch_size, num_anchors, -1)
#         batch_box_preds = torch.cat((batch_box_preds, batch_box_preds_multi), dim=1)
...
#             dir_cls_preds_multi = dir_cls_preds_multi.view(batch_size, num_anchors, -1) if not isinstance(dir_cls_preds_multi, list) \
#                 else torch.cat(dir_cls_preds_multi, dim=1).view(batch_size, num_anchors, -1)
#             dir_cls_preds = torch.cat((dir_cls_preds, dir_cls_preds_multi),dim=1)
...
class AnchorHeadTemplate(nn.Module):
    ...
    def generate_predicted_boxes(self, batch_size, cls_preds, box_preds, dir_cls_preds=None):
        ...
        # print('dir_cls_preds:,',dir_cls_preds.size())
        if isinstance(self.anchors, list):
            if self.use_multihead:
                anchors = torch.cat([anchor.permute(3, 4, 0, 1, 2, 5).contiguous().view(-1, anchor.shape[-1])
                                     for anchor in self.anchors], dim=0)
