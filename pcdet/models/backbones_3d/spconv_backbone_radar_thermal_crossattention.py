...
# from .thermal_backbone import RCNN
# from pcdet.models.backbones_2d.base_bev_backbone_VGG16FPN import FPN, BasicBlock
...
class VoxelBackBone8x_Radar_Thermal_CA(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        if self.use_attention:
            # self.attention1 = CrossAttention(16, 16, 16)
            # self.attention2 = CrossAttention(32, 32, 16)
            # self.attention3 = CrossAttention(64, 64, 16)
            # self.attention4 = CrossAttention(64, 64, 16)
            # self.attention1 = Channel_Attention(16)
            # self.attention2 = Channel_Attention(32)
            # self.attention3 = Channel_Attention(64)
            # self.attention4 = Channel_Attention(64)
    ...
    def process_attention(self, sp_tensor, img_tensor, attention_head, batch_dict, cur_stride):
        ...
        for batch_id in range(batch_size):
            ...
            if self.use_attention: 
                # voxel_features_sparse = torch.unsqueeze(voxel_features_sparse, 0).permute(0, 2, 1).contiguous()
                # image_features_batch = torch.unsqueeze(image_features_batch, 0).permute(0, 2, 1).contiguous()
                
                image_with_voxelfeatures_batch = attention_head(voxel_features_sparse, image_features_batch)
                
                # image_with_voxelfeatures_batch = torch.squeeze(image_with_voxelfeatures_batch, 0).permute(1,0).contiguous()
    ...
    def forward(self, batch_dict):
        ...
        x_conv1 = self.process_attention(x_conv1, img_conv1, self.attention1, batch_dict, cur_stride=1)
        # x_conv1: 16 + 16
        
        x_conv2 = self.conv2(x_conv1)
        x_conv2 = self.process_attention(x_conv2, img_conv2, self.attention2, batch_dict, cur_stride=2)
        
        # x_conv2: 32 + 16
        
        x_conv3 = self.conv3(x_conv2)
        x_conv3 = self.process_attention(x_conv3, img_conv3, self.attention3, batch_dict, cur_stride=4)

        
        # x_conv3: 64 + 16
        
        x_conv4 = self.conv4(x_conv3)
        x_conv4 = self.process_attention(x_conv4, img_conv4, self.attention4, batch_dict, cur_stride=8)
...
class Channel_Attention(nn.Module):
    ...
    def forward(self, x1, x2):
        avg_out = self.fc(self.avg_pool(x1))
...
class DotProductAttention(nn.Module):
    ...
    def forward(self, lidar, camera):
        query = self.query(lidar)
        key, value = self.key(camera), self.value(camera)
        # query, key, value = lidar, camera, camera
        d_k = query.size(-1)
...
class CrossAttention(nn.Module):
    def __init__(self, lidar_channels, image_channels, qkv_channels=128, num_heads=1, dropout=0):
        super().__init__()
    ...
    def _init_qkv(self):
        self.query = nn.Linear(in_features=self.lidar_channels, out_features=self.qkv_channels, bias=False)
        self.key = nn.Linear(in_features=self.image_channels, out_features=self.qkv_channels, bias=False)
        self.value = nn.Linear(in_features=self.image_channels, out_features=self.qkv_channels, bias=False)
        self.multiheadattention = nn.MultiheadAttention(self.qkv_channels, self.num_heads, dropout=self.dropout, batch_first=True)
        self.fc = nn.Linear(in_features=self.qkv_channels, out_features=self.lidar_channels, bias=False)
        # self.sfmax = nn.Softmax()
        self.sfmax = nn.ReLU()
    
    def forward(self, lidar_feature, img_feature):
        query = self.query(lidar_feature)
        key = self.key(img_feature)
        value = self.value(img_feature)
        attn_output, _ = self.multiheadattention(query, key, value)
        attn_output = self.fc(attn_output)
        attn_output = self.sfmax(attn_output)
        
        lidar_feature = torch.cat([lidar_feature, attn_output], dim=-1)

        return lidar_feature


class VoxelBackBone8x_Radar_Thermal_Concat(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        point_cloud_range=[-2, -25.6, 0, 2, 25.6, 51.2]
        voxel_size = [0.1, 0.05, 0.05]
...
class VoxelBackBone8x_Radar_Thermal_Sum(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        super().__init__()
        point_cloud_range=[-2, -25.6, 0, 2, 25.6, 51.2]
        voxel_size = [0.1, 0.05, 0.05]
